...
//   display: flex;
//   justify-content: ${({ showSelect }) => (showSelect ? 'space-between' : 'flex-end')};
//   align-items: center;
//   margin: 1rem 0;
//   height: 99px;
// `;
...
// const PostActions = ({ writePath, showSelect }) => {
//   const navigate = useNavigate();

//   return (
//     <ActionsWrapper showSelect={showSelect}>
//       {showSelect && <SelectCategory />}
//       <WriteButton onClick={() => navigate(writePath)}>
...
const PostActions = ({ writePath, showSelect }) => {
  ...
  const handleClick = () => {
    console.log(`Navigating to: ${writePath}`); // 디버깅용 로그
    navigate(writePath);
  };
